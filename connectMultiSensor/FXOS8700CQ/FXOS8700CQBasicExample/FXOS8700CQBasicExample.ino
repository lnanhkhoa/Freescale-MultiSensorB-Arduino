...
void setup()
{
  ...
  Serial.println("FXOS8700CQ 3-axis 14-bit accelerometer, 3-axis 16-bit magnetometer 0.25 mg, 1 mG.");
  // delay(2000);
  // Read the WHO_AM_I register, this is a good test of communication
  byte c = readByte(FXOS8700CQ_ADDRESS, WHO_AM_I); // Read WHO_AM_I register
  Serial.print("FXOS8700CQ I Am 0x");
  Serial.print(c, HEX);
  Serial.println(". I Should be 0xC7.");
  delay(1000);
  if (c == 0xC7)
    // WHO_AM_I should always be 0xC7
  {
    ...
    Serial.print((int16_t) (((float) mxbias) * 10./32.768));
    Serial.println(" mG");
    Serial.print("my bias ");
    Serial.print((int16_t) (((float) mybias) * 10./32.768));
    Serial.println(" mG");
    Serial.print("mz bias ");
    Serial.print((int16_t) (((float) mzbias) * 10./32.768));
    Serial.println(" mG");
    delay(1000);
  }
  else
...
void loop()
{
  ...
  if (readByte(FXOS8700CQ_ADDRESS, DR_STATUS) & 0x08)
    // When this bit set, all accel axes have new data
  {
  ...
  if (readByte(FXOS8700CQ_ADDRESS, M_DR_STATUS) & 0x08)
    // When this bit set, all mag axes have new data
  {
  ...
  // {
    ...
    Serial.println(1000. * az); //Serial.print("\t");
    ...
    // Serial.print(1000. * mx); Serial.print("\t");
    // Serial.print(1000. * my); Serial.print("\t");
    // Serial.print(1000. * mz); Serial.print("\t");
  ...
  // if(!sleepMode) {
  // Serial.println("entering sleep mode"); Serial.println("sleep on");
  // sleepMode = true;
  // }
  // else {
  // Serial.println("exiting sleep mode"); Serial.println("active");
  // sleepMode = false;
  // }
...
void tapHandler()
{
  ...
  {
    if (source & 0x08)
      // If DPE (double pulse) bit is set
...
void motionDetect()
{
  ...
  if ((source >> 7) == 1)
  {
    ...
    if (source & 0x02)
      // If XHE bit is set, x-motion detected
    {
      if (source & 0x01)
      {
    ...
    if ((source & 0x08) == 0x08)
      // If YHE bit is set, y-motion detected
    {
      if (source & 0x04)
      {
        // If YHP is set, y event was negative g
        Serial.println(" -");
    ...
    if (source & 0x20)
      // If ZHE bit is set, z-motion detected
    {
      if (source & 0x10)
      {
        // If ZHP is set
...
void calibrateFXOS8700CQ()
{
  ...
  for (ii = 0; ii < fcount; ii++)
    // construct count sums for each axis
  {
...
void initFXOS8700CQ()
{
  ...
  if (accelODR <= 7)
    writeByte(FXOS8700CQ_ADDRESS, CTRL_REG1, readByte(FXOS8700CQ_ADDRESS, CTRL_REG1) | (accelODR << 3));
